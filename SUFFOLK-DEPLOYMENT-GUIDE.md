...
**🟢 SESSION 7 (2026-05-29) — TOKEN-ECONOMY SWEEP + KEY ROTATION.** Brain page count: **147 total / 125 slack obs**, real channel coverage 2/71 (`callrail-new-daily-calls` 106, `expenses` 10). Last real Slack page was 2026-05-28 09:11 — runs since were either MCP-stale or 429-walled. Three issues found + fixed today:

1. **slack_native MCP went stale after ~8h running.** The long-running node subprocess kept booting clean ("SafeClaw Slack MCP running in reader mode") but tool calls silently returned empty-error `ClosedResourceError`. Fix = `docker compose restart hermes-reader` — recycles the subprocess. **Open follow-up:** add a daily reader restart cron OR a stdio-MCP watchdog so this self-heals.

2. **Fallback key `goofy_hugle_463` (`605722…`) was revoked** — direct probe returned `HTTP 401 unauthorized` (not 429). So the session-6 "2-key pool" was effectively a 1-key pool. Replaced with a brand-new working key (`bf30…`, verified `HTTP 200` on direct chat).

3. **`naughty_bose_129` was vaporizing weekly credit because compression was OFF.** With `auxiliary.compression.enabled: false` (commit `01beef8` — the only-reason-for-it disabled was OpenClaw concurrency starvation, which was resolved session 6), every agent turn re-sent the full growing conversation: O(n²) tokens/run. Observed **94-turn runs sending 61,485 tokens in a SINGLE LLM call**; 24× hourly runs torched the weekly cap in ~1 day. Compression now ON; expected 5-10× saving per run.

**Changes shipped (commit `39c0210`):**
- `auxiliary.compression.enabled: true` (reverts `01beef8` — original reason gone)
- `agent.max_turns: 25` (bounds the runaway wide-channel walk; was default 90; watermarks carry across runs)
- `agent.api_max_retries: 2` (skips the wasted 3rd retry on hard 429 walls)
- `schedules.slack_ingest.cron: 0 */2 * * *` (every 2h — was hourly; cadence history annotated in the yaml comment)

**Key swap (on-box, NOT committed — secrets):**
- `OLLAMA_API_KEY` in `.env` is now **`bf30…`** (fresh, has quota) — primary in pool.
- `naughty_bose_129` (`e292aa…`) demoted to **priority-1 fallback** in `/opt/data/auth.json`, marked `exhausted` so the pool skips it until its weekly cap clears.
- Dead `goofy_hugle_463` (`605722…`) **removed entirely**. Backups: `/opt/data/auth.json.bak.session7.*` and `/opt/safeclaw/.env.bak.session7.*`.

**Verified end-to-end:** post-swap cron tick `loaded credential pool for provider ollama-cloud with 2 entries`, MCP boot clean, Brookhaven `{"status":"ok"}`. Test-run results in §9.

**Operational steady-state going forward:** 12 runs/day × bounded max-turns × compressed context. `bf30…` should last until refresh; if it starts 429ing, the pool auto-rotates to `naughty_bose_129` (which by then should have weekly-reset).
...
**TWO-KEY CREDENTIAL POOL (set up session 6) — auto-rotates when a key's session usage limit (429) is hit, ~2× throughput.** Hermes pools multiple keys per provider and rotates on 429. Both Ollama keys are now in the `ollama-cloud` pool: #1 `OLLAMA_API_KEY` (env, old key `goofy_hugle_463`) + #2 `naughty_bose_129` (new key, stored in `/opt/data/auth.json` — persisted on the `safeclaw_hermes_reader_data` volume). Verified: run logs `loaded credential pool for provider ollama-cloud with 2 entries`; pages climbing (87→89→…).
- **Add/inspect pool keys:** `docker compose exec -u 10000 hermes-reader /opt/hermes/.venv/bin/hermes auth add ollama-cloud --type api-key --api-key <KEY> --label <name>`; `… hermes auth list`; clear 429 flags with `… hermes auth reset ollama-cloud`.
- **GOTCHA:** the credential pool is loaded at gateway **startup**, so after `hermes auth add` you MUST `docker compose up -d --force-recreate hermes-reader` for the new key to take effect (a long-running gateway keeps its cached 1-entry pool).
- For full-speed one-pass ingest, still better to upgrade a plan or use Anthropic; the 2-key pool just doubles the budget per window.
...
- **2026-05-29 (session 7) — diagnosed why the brain stopped growing + shipped a token-economy sweep + rotated keys.** Found three independent problems blocking ingest since 2026-05-28 09:11: (1) the `slack_native` MCP node subprocess goes silent after ~8h running — boots clean every time but tool calls return empty-error `ClosedResourceError`. Fix = restart the reader to recycle it (`docker compose restart hermes-reader`). Long-term follow-up: add a stdio-MCP watchdog or daily reader restart so this self-heals. (2) `goofy_hugle_463` fallback key (`605722…`) was revoked — direct probe returned `HTTP 401 unauthorized`; the session-6 "2-key pool" had degenerated to a 1-key pool. Replaced with a new working key (`bf30…`, `HTTP 200` verified). (3) `naughty_bose_129` (`e292aa…`) was vaporizing weekly credit because **compression had been disabled** (commit `01beef8`, session 6 — to dodge OpenClaw-induced concurrency starvation). OpenClaw was already off, so the disable was no longer needed — but the side-effect was every agent turn re-sending the full growing conversation: **O(n²) tokens/run**. Observed 94-turn cron runs sending **61,485 tokens in a single LLM call**; 24× hourly runs torched the weekly cap in ~1 day. **Commit `39c0210`** (`fix(reader): token-economy sweep`) re-enables `auxiliary.compression.enabled: true`, adds `agent.max_turns: 25` (was default 90 — bounds the runaway wide-channel walk; watermarks carry across runs), `agent.api_max_retries: 2` (was 3 — skips the wasted 3rd retry on hard 429 walls), and moves `schedules.slack_ingest.cron` to `0 */2 * * *` (12 runs/day). Cadence history is annotated in the schedule comment. Expected combined effect: 5-10× per-run token reduction × 50% fewer runs = ~10-20× weekly token budget headroom. **Key swap (on-box, NOT committed):** `.env` `OLLAMA_API_KEY` is now `bf30…` (primary); `/opt/data/auth.json` priority-1 holds `naughty_bose_129`'s `e292aa…` as the cooldown-respecting fallback; the dead `605722…` was removed. Backups at `/opt/data/auth.json.bak.session7*` and `/opt/safeclaw/.env.bak.session7.*`. Post-swap verification: pool reload logs `loaded credential pool for provider ollama-cloud with 2 entries`, slack_native MCP boots clean, Brookhaven `{"status":"ok"}` throughout (prime directive intact). Brain page count at session start: 147 total / 125 slack obs / 2 channels with real coverage (callrail-new-daily-calls 106, expenses 10). Suffolk box clone still tracks `feat/safeclaw-brain-gbrain`; HEAD is `39c0210`. Probed alternative paths (Gemma on-host) and rejected: 7.8 GiB RAM / 0 swap / 2 vCPU / no GPU box can't safely host gemma3:4b without OOM-risk to Brookhaven, and the 1-2B variants are too weak at structured tool-calling. Recommended-but-deferred long-term path remains hosted Anthropic key (no weekly caps).
