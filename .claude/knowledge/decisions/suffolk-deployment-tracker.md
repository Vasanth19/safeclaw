...
- 2026-05-22: hermes GHCR image made public by user → pulled to box (3.31GB). Onboarding UI built + started (only that container); /dashboard /setup /health all 200 on 127.0.0.1:8080. Brookhaven still 200/ok. Access pre-DNS via SSH tunnel: `ssh -L 8080:127.0.0.1:8080 suffolk-vps` → http://localhost:8080/dashboard.
- 2026-05-22: TEMP public exposure of the dashboard (close after setup). SEPARATE nginx listener on :8443 (file `/etc/nginx/sites-available/safeclaw-admin`, reuses Brookhaven cert), did NOT touch Brookhaven's vhost. Hostinger cloud firewall already allows 8443 externally. Started with basic-auth but switched to **IP allow-list** (`allow 2600:1008:a034:985b::/64; allow 97.242.154.1; deny all;`) because the operator's custom browser didn't send basic-auth on fetch() → wizard XHRs 401'd → "network error". NOTE: operator is on mobile IPv6 (may rotate; re-add the /64 if access drops). URL (passwordless from allowed IP): https://srv1687869.hstgr.cloud:8443/setup. Slack creds saved (bot+app token); workspace TLL1P1QU9. TEARDOWN: `rm /etc/nginx/sites-enabled/safeclaw-admin && nginx -t && systemctl reload nginx`. Still gated for full stack: Composio user ID, LLM key, Telegram token.
