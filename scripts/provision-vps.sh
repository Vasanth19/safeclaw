...
#   7. Start ONLY the onboarding webapp (other services boot when the
#      customer submits the setup form — they need customer credentials)
#   8. Print the URL to send to the customer + an SSH cheat-sheet
#   9. Health-check https://<DOMAIN>/health
#
# Usage:
...
if [[ -z "${DOMAIN}" || -z "${ADMIN_EMAIL}" ]]; then
  cat <<USAGE >&2
Usage: bash scripts/provision-vps.sh <domain> <admin-email>

Example:
  bash scripts/provision-vps.sh customer1.safeclaw.com hello@example.com

The domain must already have an A record pointing at this VPS.
The email is used by Caddy as the Let's Encrypt ACME contact.
USAGE
  exit 1
fi
...
apt-get install -y \
  curl git gnupg lsb-release ca-certificates ufw caddy jq \
