...
# CUSTOMER: Fill in the Composio account UUIDs for each inbox in your environment
case "$INBOX" in
    personal)
        # CUSTOMER: Replace with your personal Gmail account UUID from Composio dashboard
        COMPOSIO_ACCOUNT_UUID="__FILL_IN_PERSONAL_UUID__"
        ;;
    work)
        # CUSTOMER: Replace with your work Gmail account UUID from Composio dashboard
        COMPOSIO_ACCOUNT_UUID="__FILL_IN_WORK_UUID__"
        ;;
    sales)
        # CUSTOMER: Replace with your sales Gmail account UUID from Composio dashboard
        COMPOSIO_ACCOUNT_UUID="__FILL_IN_SALES_UUID__"
        ;;
    *)
        echo "ERROR: invalid inbox '$INBOX'. Must be one of: personal, work, sales"
        echo "CUSTOMER: Add more inboxes here and fill in their Composio UUIDs in the script"
        exit 1
        ;;
esac

# Validate that placeholder was filled in
if [[ "$COMPOSIO_ACCOUNT_UUID" == __FILL_IN* ]]; then
    echo "ERROR: Composio account UUID not configured for inbox '$INBOX'"
    echo "CUSTOMER: Edit this script and replace the __FILL_IN_* placeholders with your actual UUIDs"
    echo "         You can find these in the Composio dashboard at https://app.composio.dev"
    exit 1
fi
...
def classify(msg, replied_thread_ids, brain_people_emails):
    ...
    # Tier 1b: SOCIAL → LOG (mitigation #3 — keep LinkedIn InMail queryable)
...
# CUSTOMER: Update the container name to match your docker compose setup
# Default: {project_slug}-postgres-obs (e.g. safeclaw-postgres-obs)
container_name = os.environ.get('POSTGRES_CONTAINER_NAME', 'safeclaw-postgres-obs')
